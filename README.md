...
In fact, this is designed to be a game : the users who want to create a new block have to resolve a puzzle.
...
At start, during the adoption phase, the complexity was 4, meaning that the user is searching a value of `nonce` that makes a hash that starts with 4 zeros. This is easy and doesn't cost so much of time and CPU.
...
In order to encourage users to mine (as CPU and time required can limit enthusiasm), there is a counterpart : when a user succesfully mine a block, then the blockchain add a `reward` to the user in the transactions of the block.
This reward is a special transaction as there is no emitter : the money is generated from nowhere!
Similarly to `complexity`, the `reward` also change over time : it decrease with the count of users : at start, the blockchain needs to attract users, so the compensation is great. But after time, there is a lot of potential users, which increase the probability for a block to be mined by someone. In order to limit the count of potential users, the reward is reduced.
...
3. When `mallory` also updates each block's `proof of work`, the chain is valid again. But the `test6` takes time.
Let's modify the `Blockchain.complexity` to 5 or 6 if you're not convinced. Let's imagine 32!
